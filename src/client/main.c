...
#define	PD_ISSET(n, p) ((p)->__fds_bits[(n)/FDBITS] & (1 << ((n) % FDBITS)))
 ...
 */
static int console_lock(int domid)
{
...
int main(int argc, char **argv)
{
	...
	if (argc >5 || optind >= argc || optind == -1) {
	...
	if (xs == NULL) {
		...
		fprintf(stderr, "Could not conact Xenstore\n");
		exit(EINVAL);
	}
	...
	path = malloc(10 + strlen("/device/console/0/tty") + 5);
	if (path == NULL) {
		for (int n=0; n<3; n++)
                      	free(argv[n]);
		free(argv);
	...
	if (type == CONSOLE_PV) {
		...
		if (num == 0)
			snprintf(path, 25 + 13, "%s/console/tty", dom_path);
			//snprintf(path, strlen(dom_path) + strlen("/console/tty") + 1, "%s/console/tty", dom_path);
		else
			snprintf(path, 22 + 5, "%s/device/console/%d/tty", dom_path, num);
